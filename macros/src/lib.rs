...
pub fn dyn_trait(_: proc::TokenStream, input: proc::TokenStream) -> proc::TokenStream {
    ...
    let to_dyn_trait = quote::format_ident!("dyn_{}", trait_name.to_string().to_lowercase());
...
Generated by [macros::dyn_trait]
    ...
    let dyn_trait = quote! {
        ...
        pub trait #dyn_trait_name {
            ...
            fn #to_dyn_trait(&self) -> &dyn #trait_name;
        }

        impl<T: #trait_name + Clone + 'static> #dyn_trait_name for T {
...
type Implemented = HashMap<&'static str, TokenStream>;

fn basic(name: &syn::Ident) -> Implemented {
    let s = quote! {
        impl Basic for #name {
    ...
    let mut imp = Implemented::new();
    imp.insert("Basic", s);
    imp
}

// macro_rules! impl_derive_macro {
//     ($trt_name: ident) => {
//         paste! {
//             #[proc_macro_derive($trt_name)]
//             pub fn [<$trt_name:snake _derive>](item: proc::TokenStream) -> proc::TokenStream {
//                 let name = &parse_macro_input!(item as ItemStruct).ident;
//                 let imp = [<$trt_name:snake>](name);
//                 let mut s = TokenStream::new();
//                 imp.into_iter().for_each(|(_, stream)| s.extend(stream));
//                 s.into()
//             }
//         }
//     };
...
// macro_rules! impl_trait_gen {
//     ($trt_name: ident: $prev1: ident $(+ $prev2: ident)*) => {
//         paste! {
//             fn [<$trt_name:snake>](name: &syn::Ident) -> Implemented {
//                 let s = quote! {
//                     impl $trt_name for #name {}
//                 };

//                 #[allow(unused_mut)]
//                 let mut imp = [<$prev1:snake>](name);
//                 imp.insert(stringify!($trt_name), s);

//                 $(
//                 let imp2 = [<$prev2:snake>](name);
//                 imp.extend(imp2);
//                 )*
...
//         impl_derive_macro!($trt_name);
//         }
//     };

//     ($trt_name: ident [user]: $prev: ident) => {
//         paste! {
//             fn [<$trt_name:snake>](name: &syn::Ident) -> Implemented {
//                 let s = quote! {};
//                 let mut imp = [<$prev:snake>](name);
//                 imp.insert(stringify!($trt_name), s);
//                 imp
//             }
//         }
...
// impl_trait_gen!(Atom: Basic);
// impl_trait_gen!(Expr: Basic);
// impl_trait_gen!(Boolean[user]: Basic);
// impl_trait_gen!(Application: Basic);
// impl_trait_gen!(AtomicExpr: Atom + Expr);
// impl_trait_gen!(BooleanAtom: Atom + Boolean);
// impl_trait_gen!(BooleanFunc: Boolean + Application);

#[proc_macro_derive(Basic)]
pub fn basic_derive(item: proc::TokenStream) -> proc::TokenStream {
    let name = &parse_macro_input!(item as ItemStruct).ident;
    basic(name).remove("Basic").unwrap().into()
}
